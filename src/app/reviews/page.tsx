...
export default function ReviewsPage() {
  return (
    <div className="min-h-screen pt-20 bg-surface-container-lowest">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {[
            ...
            <div key={s.label} className="bg-surface-container-lowest rounded-2xl p-5 text-center shadow-card border border-outline-variant/20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((r) => (
            <div key={r.name} className="bg-surface-container-lowest rounded-3xl p-6 shadow-card border border-outline-variant/20 card-hover">
              ...
              <Quote size={24} className="text-tertiary/20 mb-3" />
              <p className="text-on-surface-variant text-sm leading-relaxed mb-5">&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center gap-3 pt-4 border-t border-outline-variant/10">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-tertiary flex items-center justify-center text-white font-bold text-xs shrink-0">
                  {r.avatar}
                </div>
                <div>
                  <p className="font-bold text-on-surface text-sm">{r.name}</p>
                  <p className="text-xs text-on-surface-variant">{r.country} · {r.subject} · {r.grade}</p>
                </div>
              </div>
            </div>
          ))}
