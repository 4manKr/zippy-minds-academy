...
export default function LoginPage() {
  ...
  const handleGetCode = () => {
    if (!email) return;
    setLoading(true);
    setTimeout(() => { setCodeSent(true); setLoading(false); }, 800);
  };
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="mb-6 text-center">
          <div className="relative w-20 h-20 mx-auto mb-4">
            <Image src="/zippy-logo.jpeg" alt="Zippy Minds Academy" fill className="object-contain rounded-2xl shadow-card" />
          </div>
          <h1 className="font-display text-2xl md:text-3xl font-extrabold text-primary leading-tight">
            Welcome back, Zippy Explorer!
          </h1>
        ...
        <div className="w-full max-w-md bg-surface-container-lowest rounded-3xl shadow-card-hover border border-outline-variant/40 p-8">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-on-surface mb-1.5">Email or Mobile Number</label>
              <div className="relative">
                <Mail size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-on-surface-variant" />
                <input
                  type="email"
                  placeholder="e.g. kid@zippy.com"
                  value={email}
            ...
            <div>
              ...
              {codeSent && (
                <p className="text-xs text-primary mt-1.5 font-medium">Code sent! Check your email or SMS.</p>
              )}
            </div>

            <Link href={role === "tutor" ? "/dashboard/tutor" : "/dashboard/parent"} className="btn-primary w-full justify-center py-3.5 text-sm mt-2 rounded-xl">
          ...
          <Link href="/book-demo" className="btn-yellow w-full justify-center py-3.5 text-sm rounded-xl">
            Book a Free Demo
          </Link>
          ...
          {role === "tutor" && (
            <p className="text-center text-xs text-on-surface-variant mt-2">
              Not yet approved?{" "}
              <Link href="/auth/tutor-login" className="text-tertiary font-semibold hover:underline">Tutor portal →</Link>
            </p>
          )}
        </div>
      </main>
