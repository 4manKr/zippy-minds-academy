...
const faqs = [
  ...
  { q: "What are the pricing options?",         a: "Session prices vary by tutor and subject, typically ranging from ₹1,200–₹2,800 per hour. We also offer subscription packages for ongoing learning at discounted rates." },
          ...
          {faqs.map((faq, index) => (
            <div
              ...
              className={cn(
                ...
                openIndex === index ? "border-primary/30 shadow-sm" : "border-outline-variant/20"
              )}
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-surface-container-lowest transition-colors"
              >
